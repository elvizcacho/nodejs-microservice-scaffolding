...
export class ApplicationLogger {
  ...
  getAppLabel(): string {
    return this.config.appLabel
      ? `${this.config.appLabelTextColor}${this.config.appLabel}${TextColor.RESET}`
      : ''
  }

  getClassLabel(): string {
    return this.config.classLabel
      ? `${this.config.classLabelTextColor}${this.config.classLabel}${TextColor.RESET}`
      : ''
  }
