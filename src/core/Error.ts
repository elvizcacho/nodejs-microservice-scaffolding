...
export class InvalidOperationError extends Error implements ApplicationError {
  constructor(message: string, origin?: string, public code: TStatusCode = 400) {
...
const getErrorResponseByStatusCode = (code: number, token?: TokenString) => {
  switch (code) {
    case 401:
      return new ErrorResponse(token, Errors.INVALID_TOKEN, 'authorization', ErrorLocation.HEADER);
    case 404:
    ...
    default:
      return new ErrorResponse('', Errors.WRONG_INPUT_DATA, '', ErrorLocation.BODY);
  }
};

export const errorToResponse = (e: InvalidOperationError, token?: string): { errorsResponse: ErrorsResponse; code: number } => {
